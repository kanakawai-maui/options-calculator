...
export function TickerScreener({ open, onClose, onSelect, user, onSignIn }) {
  ...
  // ── Premium gate: require sign-in ────────────────────────────────────────
  if (!user) {
    return (
      <div className="ts-overlay" role="dialog" aria-modal="true" aria-label="Ticker screener">
        <div className="ts-backdrop" onClick={onClose} />
        <div className="ts-dialog ts-dialog--gate">
          <button type="button" className="ts-close" onClick={onClose} aria-label="Close screener">
            ✕
          </button>
          <div className="ts-gate">
            <div className="ts-gate-icon" aria-hidden="true">🔒</div>
            <h2 className="ts-gate-title">Premium Feature</h2>
            <p className="ts-gate-body">
              The Ticker Screener scans 40+ option chains for IV, skew, and
              liquidity. Sign in with Google to unlock it — it’s free.
            </p>
            <button
              type="button"
              className="ts-gate-btn"
